"""Mirror of pi coding-agent src/core/tools/renderers/find.ts.
...
`find.py` spreads these into its definition, so the tool's public shape is
unchanged.
"""
...
find_renderers = ToolRenderers(render_call=_render_call, render_result=_render_result)

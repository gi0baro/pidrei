...
def _write_escape(ctx, sequence: str) -> None:
    if ctx.has_ui:
        # While the TUI runs, its output pump is the terminal's only writer: a
        # direct stdout write could land inside a frame (pi's
...
def _notify_osc777(ctx, title: str, body: str) -> None:
    _write_escape(ctx, f"\x1b]777;notify;{title};{body}\x07")


def _notify_osc99(ctx, title: str, body: str) -> None:
    # Kitty OSC 99: i=notification id, d=0 means not done yet, p=body for second part
    _write_escape(ctx, f"\x1b]99;i=1:d=0;{title}\x1b\\")
    _write_escape(ctx, f"\x1b]99;i=1:p=body;{body}\x1b\\")

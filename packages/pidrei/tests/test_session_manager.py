...
class TestAppendOperations:
    def test_append_message_creates_entry_with_correct_parent_chain(self):
    ...
    def test_append_thinking_level_change_integrates_into_tree(self):
        session = SessionManager.in_memory()
    ...
    def test_append_model_change_integrates_into_tree(self):
        session = SessionManager.in_memory()
    ...
    def test_append_compaction_integrates_into_tree(self):
        session = SessionManager.in_memory()
    ...
    def test_append_custom_entry_integrates_into_tree(self):
        session = SessionManager.in_memory()
    ...
    def test_leaf_pointer_advances_after_each_append(self):
        session = SessionManager.in_memory()
...
class TestGetPath:
    ...
    def test_returns_single_entry_path(self):
        session = SessionManager.in_memory()
    ...
    def test_returns_full_path_from_root_to_leaf(self):
        session = SessionManager.in_memory()
    ...
    def test_returns_path_from_specified_entry_to_root(self):
        session = SessionManager.in_memory()
...
class TestGetTree:
    ...
    def test_returns_single_root_for_linear_session(self):
        session = SessionManager.in_memory()
    ...
    def test_returns_tree_with_branches_after_branch(self):
        session = SessionManager.in_memory()
    ...
    def test_handles_multiple_branches_at_same_point(self):
        session = SessionManager.in_memory()
    ...
    def test_handles_deep_branching(self):
        session = SessionManager.in_memory()
...
class TestBranch:
    def test_moves_leaf_pointer_to_specified_entry(self):
        session = SessionManager.in_memory()
    ...
    def test_throws_for_non_existent_entry(self):
        session = SessionManager.in_memory()
    ...
    def test_new_appends_become_children_of_branch_point(self):
        session = SessionManager.in_memory()
...
class TestBranchWithSummary:
    def test_inserts_branch_summary_and_advances_leaf(self):
        session = SessionManager.in_memory()
...
class TestLeafAndEntryAccess:
    ...
    def test_get_leaf_entry_returns_current_leaf(self):
        session = SessionManager.in_memory()
    ...
    def test_get_entry_returns_entry_by_id(self):
        session = SessionManager.in_memory()
...
class TestBuildSessionContextWithBranchesManager:
    def test_returns_messages_from_current_branch_only(self):
        session = SessionManager.in_memory()
...
class TestCreateBranchedSession:
    ...
    def test_creates_new_session_with_path_to_leaf_in_memory(self):
        session = SessionManager.in_memory()
    ...
    def test_extracts_correct_path_from_branched_tree(self):
        session = SessionManager.in_memory()
    ...
    def test_does_not_duplicate_entries_when_forking_from_first_user_message(self, tmp_path):
        temp_dir = str(tmp_path)
        session = SessionManager.create(temp_dir, temp_dir)
    ...
    def test_preserves_tool_and_summary_usage_across_file_backed_reload(self, tmp_path):
        temp_dir = str(tmp_path)
        session = SessionManager.create(temp_dir, temp_dir)
        root_id = session.append_message(user_msg("question"))
        session.append_message(assistant_msg("answer"))
        usage = make_usage()
        session.append_message(tool_result_msg("result", usage=usage))
        ...
        reopened = SessionManager.open(file, temp_dir)
    ...
    def test_writes_file_immediately_when_forking_from_point_with_assistant(self, tmp_path):
        temp_dir = str(tmp_path)
        session = SessionManager.create(temp_dir, temp_dir)
...
class TestSaveCustomEntry:
    def test_saves_custom_entries_and_includes_in_tree_traversal(self):
        session = SessionManager.in_memory()

        msg_id = session.append_message(UserMessage(content="hello", timestamp=1))
        custom_id = session.append_custom_entry("my_data", {"foo": "bar"})
        msg2_id = session.append_message(assistant_msg("hi", timestamp=2))
...
class TestCustomSessionId:
    ...
    def test_uses_provided_id_when_creating_persisted_session(self, tmp_path):
        temp_dir = str(tmp_path)
        session = SessionManager.create(temp_dir, temp_dir, {"id": "created-session-id"})
    ...
    def test_generates_uuidv7_when_creating_branched_session(self):
        session = SessionManager.in_memory()
        first_id = session.append_message(user_msg("hello"))
    ...
    def test_generates_uuidv7_when_forking_from_session_file(self, tmp_path):
        temp_dir = str(tmp_path)
        ...
        forked = SessionManager.fork_from(source_path, temp_dir, temp_dir)
        header = forked.get_header()
        assert header is not None
        assert UUID_V7_RE.match(header["id"])
        assert header["parentSession"] == source_path

    def test_uses_provided_id_when_forking_from_session_file(self, tmp_path):
        temp_dir = str(tmp_path)
        source_path = os.path.join(temp_dir, "source.jsonl")
        with open(source_path, "w", encoding="utf-8") as handle:
            handle.write(
        ...
        forked = SessionManager.fork_from(source_path, temp_dir, temp_dir, {"id": "forked-session-id"})
        header = forked.get_header()
...
class TestLoadEntriesFromFile:
    ...
    def test_reads_cwd_from_session_headers(self, tmp_path, prefix, session_id):
        temp_dir = str(tmp_path)
        ...
        session_manager = SessionManager.open(file, temp_dir)
        assert session_manager.get_session_id() == session_id
        assert session_manager.get_cwd() == stored_cwd

    def test_opens_compatible_sessions_beyond_discovery_scan_limit(self, tmp_path):
        temp_dir = str(tmp_path)
        stored_cwd = os.path.join(temp_dir, "stored-project")
        ...
        for name, session_id, prefix in cases:
            ...
            for cwd_override in (None, override_cwd):
                session_manager = SessionManager.open(file, temp_dir, cwd_override)
                assert session_manager.get_session_id() == session_id
                assert session_manager.get_cwd() == (cwd_override if cwd_override is not None else stored_cwd)

    def test_opens_session_files_with_many_chunks(self, tmp_path):
...
def _create_persisted_session(cwd: str, session_dir: str, label: str) -> str:
    ...
    session.append_message(user_msg(label))
    session.append_message(assistant_msg(f"reply to {label}"))
    session_file = session.get_session_file()
...
async def test_scopes_current_folder_apis_by_cwd_while_listing_all_flat_sessions(tmp_dir):
    ...
    session_a = _create_persisted_session(project_a, tmp_dir, "from A")
    await tonio.time.sleep(0.01)
    session_b = _create_persisted_session(project_b, tmp_dir, "from B")
    ...
    continued_a = SessionManager.continue_recent(project_a, tmp_dir)
    assert continued_a.get_session_file() == session_a


class TestSetSessionFileCorrupted:
    def test_truncates_and_rewrites_empty_file_with_valid_header(self, tmp_path):
        empty_file = os.path.join(str(tmp_path), "empty.jsonl")
        open(empty_file, "w").close()

        sm = SessionManager.open(empty_file, str(tmp_path))
    ...
    def test_throws_and_preserves_non_empty_file_without_valid_header(self, tmp_path):
        ...
        with pytest.raises(Exception, match="Session file is not a valid pidrei session"):
            SessionManager.open(no_header_file, str(tmp_path))
        with open(no_header_file, encoding="utf-8") as handle:
            assert handle.read() == original_content

    def test_throws_and_preserves_non_session_jsonl_files(self, tmp_path):
        ...
        with pytest.raises(Exception, match="Session file is not a valid pidrei session"):
            SessionManager.open(non_session_file, str(tmp_path))
        with open(non_session_file, encoding="utf-8") as handle:
            assert handle.read() == original_content

    def test_preserves_explicit_session_file_path_when_recovering(self, tmp_path):
        explicit_path = os.path.join(str(tmp_path), "my-session.jsonl")
        open(explicit_path, "w").close()

        sm = SessionManager.open(explicit_path, str(tmp_path))
        assert sm.get_session_file() == explicit_path

    def test_subsequent_loads_of_initialized_empty_file_work(self, tmp_path):
        empty_file = os.path.join(str(tmp_path), "empty.jsonl")
        open(empty_file, "w").close()

        sm1 = SessionManager.open(empty_file, str(tmp_path))
        session_id = sm1.get_session_id()

        sm2 = SessionManager.open(empty_file, str(tmp_path))
        assert sm2.get_session_id() == session_id
        assert sm2.get_header()["type"] == "session"


class TestLabels:
    def test_sets_and_gets_labels(self):
        session = SessionManager.in_memory()
    ...
    def test_clears_labels_with_none(self):
        session = SessionManager.in_memory()
    ...
    def test_labels_are_included_in_tree_nodes(self):
        session = SessionManager.in_memory()
    ...
    def test_labels_preserved_in_create_branched_session(self):
        session = SessionManager.in_memory()
    ...
    def test_rewires_children_of_removed_labels_when_forking(self):
        session = SessionManager.in_memory()
    ...
    def test_labels_not_on_path_are_not_preserved(self):
        session = SessionManager.in_memory()
    ...
    def test_labels_not_included_in_build_session_context(self):
        session = SessionManager.in_memory()
    ...
    def test_throws_when_labeling_non_existent_entry(self):
        session = SessionManager.in_memory()

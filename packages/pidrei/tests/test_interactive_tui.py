...
class RecordingTerminal(VirtualTerminal):
    """VirtualTerminal that records every write."""
...
async def test_selects_the_alternate_screen_renderer_only_when_requested():
    ...
    main_tui = create_interactive_tui(
        alt=False, show_hardware_cursor=False, log_directory="/tmp", terminal=main_terminal
    )
    ...
    alt_tui = create_interactive_tui(alt=True, show_hardware_cursor=False, log_directory="/tmp", terminal=alt_terminal)
...
async def test_flashes_copied_for_the_copy_shortcut_in_fullscreen_mode():
    ...
    ui = create_interactive_tui(alt=True, show_hardware_cursor=False, log_directory="/tmp", terminal=terminal)
...
async def test_keeps_the_status_line_confirmation_for_the_copy_shortcut_in_regular_mode():
    ...
    ui = create_interactive_tui(
        alt=False, show_hardware_cursor=False, log_directory="/tmp", terminal=RecordingTerminal()
    )
    context = _CopyRecorder(ui)
...
@pytest.mark.parametrize(("alt", "expected_children"), [(False, 1), (True, 0)])
def test_reserves_status_height_only_on_the_main_screen_renderer(alt, expected_children):

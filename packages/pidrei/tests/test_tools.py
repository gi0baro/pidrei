...
class TestReadTool:
    @pytest.mark.tonio
    async def test_reads_file_contents_that_fit_within_limits(self, tmp_dir):
        test_file = tmp_dir / "test.txt"
        content = "Hello, world!\nLine 2\nLine 3"
        test_file.write_text(content)
        read_tool = create_read_tool(str(tmp_dir))
    ...
    @pytest.mark.tonio
    async def test_handles_non_existent_files(self, tmp_dir):
        read_tool = create_read_tool(str(tmp_dir))
        with pytest.raises(Exception, match="No such file|not found"):
            await read_tool.execute("test-call-2", {"path": str(tmp_dir / "nonexistent.txt")})

    @pytest.mark.tonio
    async def test_truncates_files_exceeding_line_limit(self, tmp_dir):
    ...
    @pytest.mark.tonio
    async def test_truncates_when_byte_limit_exceeded(self, tmp_dir):
        import re

        test_file = tmp_dir / "large-bytes.txt"
    ...
    @pytest.mark.tonio
    async def test_handles_offset_parameter(self, tmp_dir):
        test_file = tmp_dir / "offset-test.txt"
    ...
    @pytest.mark.tonio
    async def test_handles_limit_parameter(self, tmp_dir):
        test_file = tmp_dir / "limit-test.txt"
        test_file.write_text("\n".join(f"Line {i + 1}" for i in range(100)))
        read_tool = create_read_tool(str(tmp_dir))
    ...
    @pytest.mark.tonio
    async def test_handles_offset_and_limit_together(self, tmp_dir):
        test_file = tmp_dir / "offset-limit-test.txt"
        test_file.write_text("\n".join(f"Line {i + 1}" for i in range(100)))
        read_tool = create_read_tool(str(tmp_dir))
    ...
    @pytest.mark.tonio
    async def test_shows_error_when_offset_is_beyond_file_length(self, tmp_dir):
        test_file = tmp_dir / "short.txt"
    ...
    @pytest.mark.tonio
    async def test_includes_truncation_details_when_truncated(self, tmp_dir):
        test_file = tmp_dir / "large-file.txt"
        test_file.write_text("\n".join(f"Line {i + 1}" for i in range(2500)))
        read_tool = create_read_tool(str(tmp_dir))
    ...
    @pytest.mark.tonio
    async def test_detects_image_mime_type_from_file_magic_not_extension(self, tmp_dir):
        test_file = tmp_dir / "image.txt"
    ...
    @pytest.mark.tonio
    async def test_reads_bmp_files_from_disk_as_png_image_attachments(self, tmp_dir):
        test_file = tmp_dir / "image.bmp"
    ...
    @pytest.mark.tonio
    async def test_treats_files_with_image_extension_but_non_image_content_as_text(self, tmp_dir):
        test_file = tmp_dir / "not-an-image.png"
...
class TestWriteTool:
    @pytest.mark.tonio
    async def test_writes_file_contents(self, tmp_dir):
    ...
    async def test_creates_parent_directories(self, tmp_dir):
        test_file = tmp_dir / "nested" / "dir" / "test.txt"
        write_tool = create_write_tool(str(tmp_dir))
...
class TestEditTool:
    @pytest.mark.tonio
    async def test_replaces_text_in_file(self, tmp_dir):
        test_file = tmp_dir / "edit-test.txt"
        original_content = "Hello, world!"
        test_file.write_text(original_content)
        edit_tool = create_edit_tool(str(tmp_dir))
    ...
    @pytest.mark.tonio
    async def test_fails_if_text_not_found(self, tmp_dir):
        test_file = tmp_dir / "edit-test.txt"
    ...
    @pytest.mark.tonio
    async def test_includes_enoent_when_the_edit_target_does_not_exist(self, tmp_dir):
    ...
    @pytest.mark.tonio
    async def test_fails_if_text_appears_multiple_times(self, tmp_dir):
        test_file = tmp_dir / "edit-test.txt"
    ...
    @pytest.mark.tonio
    async def test_replaces_multiple_disjoint_regions_in_one_call(self, tmp_dir):
        test_file = tmp_dir / "edit-multi.txt"
    ...
    @pytest.mark.tonio
    async def test_collapses_large_unchanged_gaps_in_multi_edit_diffs(self, tmp_dir):
        test_file = tmp_dir / "edit-multi-large-gap.txt"
    ...
    @pytest.mark.tonio
    async def test_matches_edits_against_the_original_file_not_incrementally(self, tmp_dir):
        test_file = tmp_dir / "edit-multi-original.txt"
    ...
    @pytest.mark.tonio
    async def test_fails_when_edits_is_empty(self, tmp_dir):
        test_file = tmp_dir / "edit-empty-edits.txt"
    ...
    @pytest.mark.tonio
    async def test_fails_when_multi_edit_regions_overlap(self, tmp_dir):
        test_file = tmp_dir / "edit-overlap.txt"
    ...
    @pytest.mark.tonio
    async def test_does_not_partially_apply_edits_when_one_edit_fails(self, tmp_dir):
        test_file = tmp_dir / "edit-no-partial.txt"
    ...
    @pytest.mark.tonio
    async def test_includes_eacces_for_read_only_files(self, tmp_dir):
        test_file = tmp_dir / "edit-readonly.txt"
        test_file.write_text("hello\n")
    ...
    @pytest.mark.tonio
    async def test_includes_the_original_error_message_for_unknown_edit_access_errors(self, tmp_dir):
        ...
        generic_failure_tool = create_edit_tool(str(tmp_dir), operations=FailingOperations())
    ...
    @pytest.mark.tonio
    async def test_includes_enoent_in_diff_preview_for_missing_files(self, tmp_dir):
        from pidrei.core.tools.edit_diff import Edit

        missing_file = tmp_dir / "missing-preview.txt"
        result = await compute_edits_diff(str(missing_file), [Edit(old_text="hello", new_text="world")], str(tmp_dir))

        assert isinstance(result, EditDiffError)
        assert result.error == f"Could not edit file: {missing_file}. Error code: ENOENT."

    @pytest.mark.tonio
    async def test_includes_eacces_in_diff_preview_for_unreadable_files(self, tmp_dir):
        from pidrei.core.tools.edit_diff import Edit

        unreadable_file = tmp_dir / "unreadable-preview.txt"
        unreadable_file.write_text("hello\n")
        os.chmod(unreadable_file, 0o222)

        result = await compute_edits_diff(
            str(unreadable_file), [Edit(old_text="hello", new_text="world")], str(tmp_dir)
        )
...
class TestBashTool:
    ...
    async def test_includes_full_output_path_for_truncated_timeout_and_abort_errors(self, tmp_dir):
        ...
        for error_message, expected in (
            ...
            bash = create_bash_tool(str(tmp_dir), operations=ChattyFailOperations(error_message))
    ...
    @pytest.mark.tonio
    async def test_handles_process_spawn_errors(self, tmp_dir):
        ...
        try:
            bash_with_bad_shell = create_bash_tool(str(tmp_dir))
    ...
    @pytest.mark.tonio
    async def test_passes_shell_path_through_to_shell_resolution(self, tmp_dir):
    ...
    @pytest.mark.tonio
    async def test_coalesces_streaming_updates_for_chatty_output(self, tmp_dir):
        ...
        updates = []
        bash = create_bash_tool(str(tmp_dir), operations=ChattyOperations())
    ...
    @pytest.mark.tonio
    async def test_does_not_count_a_trailing_newline_as_an_extra_truncated_line(self, tmp_dir):
        import re
        ...
        bash = create_bash_tool(str(tmp_dir), operations=ManyLinesOperations())
    ...
    @pytest.mark.tonio
    async def test_decodes_utf8_characters_split_across_output_chunks(self, tmp_dir):
        euro = "€\n".encode()
        ...
        bash = create_bash_tool(str(tmp_dir), operations=SplitUtf8Operations())
...
class TestGrepTool:
    @pytest.mark.tonio
    async def test_includes_filename_when_searching_a_single_file(self, tmp_dir):
        test_file = tmp_dir / "example.txt"
        test_file.write_text("first line\nmatch line\nlast line")
        grep_tool = create_grep_tool(str(tmp_dir))
    ...
    @pytest.mark.tonio
    async def test_respects_global_limit_and_includes_context_lines(self, tmp_dir):
        test_file = tmp_dir / "context.txt"
    ...
    @pytest.mark.tonio
    async def test_treats_flag_like_patterns_as_search_text(self, tmp_dir):
        marker = tmp_dir / "grep-injection-marker"
        payload = tmp_dir / "payload.sh"
        test_file = tmp_dir / "target.txt"
        ...
        result = await grep_tool.execute(
            "test-call-grep-injection", {"pattern": f"--pre={payload}", "path": str(tmp_dir)}
        )
...
class TestFindTool:
    @pytest.mark.tonio
    async def test_includes_hidden_files_that_are_not_gitignored(self, tmp_dir):
        hidden_dir = tmp_dir / ".secret"
        hidden_dir.mkdir()
        (hidden_dir / "hidden.txt").write_text("hidden")
        (tmp_dir / "visible.txt").write_text("visible")
        find_tool = create_find_tool(str(tmp_dir))

        result = await find_tool.execute("test-call-13", {"pattern": "**/*.txt", "path": str(tmp_dir)})
    ...
    @pytest.mark.tonio
    async def test_respects_gitignore(self, tmp_dir):
        (tmp_dir / ".gitignore").write_text("ignored.txt\n")
        (tmp_dir / "ignored.txt").write_text("ignored")
        (tmp_dir / "kept.txt").write_text("kept")
        find_tool = create_find_tool(str(tmp_dir))

        result = await find_tool.execute("test-call-14", {"pattern": "**/*.txt", "path": str(tmp_dir)})
    ...
    @pytest.mark.tonio
    async def test_surfaces_fd_glob_parse_errors(self, tmp_dir):
        find_tool = create_find_tool(str(tmp_dir))
        with pytest.raises(Exception, match="error parsing glob|fd exited with code 1|fd error"):
            await find_tool.execute("test-call-15", {"pattern": "[", "path": str(tmp_dir)})

    @pytest.mark.tonio
    async def test_treats_flag_like_patterns_as_search_text(self, tmp_dir):
        find_tool = create_find_tool(str(tmp_dir))
        result = await find_tool.execute("test-call-find-flag-pattern", {"pattern": "--help", "path": str(tmp_dir)})
        assert "No files found matching pattern" in get_text_output(result)


class TestLsTool:
    @pytest.mark.tonio
    async def test_lists_dotfiles_and_directories(self, tmp_dir):
        (tmp_dir / ".hidden-file").write_text("secret")
        (tmp_dir / ".hidden-dir").mkdir()
        ls_tool = create_ls_tool(str(tmp_dir))

        result = await ls_tool.execute("test-call-15", {"path": str(tmp_dir)})
        output = get_text_output(result)
...
class TestEditToolFuzzyMatching:
    @pytest.mark.tonio
    async def test_matches_text_with_trailing_whitespace_stripped(self, tmp_dir):
        test_file = tmp_dir / "trailing-ws.txt"
    ...
    @pytest.mark.tonio
    async def test_matches_fullwidth_punctuation_in_chinese_text(self, tmp_dir):
        test_file = tmp_dir / "chinese-punctuation.txt"
    ...
    @pytest.mark.tonio
    async def test_matches_compatibility_equivalent_unicode_forms(self, tmp_dir):
        test_file = tmp_dir / "unicode-compatibility.txt"
    ...
    @pytest.mark.tonio
    async def test_matches_smart_single_quotes_to_ascii_quotes(self, tmp_dir):
        test_file = tmp_dir / "smart-quotes.txt"
    ...
    @pytest.mark.tonio
    async def test_matches_smart_double_quotes_to_ascii_quotes(self, tmp_dir):
        test_file = tmp_dir / "smart-double-quotes.txt"
    ...
    @pytest.mark.tonio
    async def test_matches_unicode_dashes_to_ascii_hyphen(self, tmp_dir):
        test_file = tmp_dir / "unicode-dashes.txt"
    ...
    @pytest.mark.tonio
    async def test_matches_non_breaking_space_to_regular_space(self, tmp_dir):
        test_file = tmp_dir / "nbsp.txt"
        test_file.write_text("hello world\n")
        edit_tool = create_edit_tool(str(tmp_dir))
    ...
    @pytest.mark.tonio
    async def test_prefers_exact_match_over_fuzzy_match(self, tmp_dir):
        test_file = tmp_dir / "exact-preferred.txt"
    ...
    @pytest.mark.tonio
    async def test_still_fails_when_text_is_not_found_even_with_fuzzy_matching(self, tmp_dir):
        test_file = tmp_dir / "no-match.txt"
    ...
    @pytest.mark.tonio
    async def test_detects_duplicates_after_fuzzy_normalization(self, tmp_dir):
        test_file = tmp_dir / "fuzzy-dups.txt"
        test_file.write_text("hello world   \nhello world\n")
        edit_tool = create_edit_tool(str(tmp_dir))
    ...
    @pytest.mark.tonio
    async def test_supports_fuzzy_matching_in_multi_edit_mode(self, tmp_dir):
        test_file = tmp_dir / "fuzzy-multi.txt"
        test_file.write_text("console.log(‘hello’);\nhello world\n")
        edit_tool = create_edit_tool(str(tmp_dir))
    ...
    @pytest.mark.tonio
    async def test_preserves_the_correct_occurrence_when_fuzzy_replacement_equals_a_nearby_line(self, tmp_dir):
        test_file = tmp_dir / "fuzzy-preserve-duplicate-line.txt"
    ...
    @pytest.mark.tonio
    async def test_preserves_untouched_lines_and_produces_an_applicable_patch_for_fuzzy_multi_edits(self, tmp_dir):
        test_file = tmp_dir / "fuzzy-preserve-multi.txt"
        original_content = (
            "keep before  \nfirst target  \nfirst after\nkeep middle   \nsecond target  \nsecond after\nkeep after  \n"
        )
        test_file.write_text(original_content)
...
class TestEditToolCrlfHandling:
    @pytest.mark.tonio
    async def test_matches_lf_old_text_against_crlf_file_content(self, tmp_dir):
        test_file = tmp_dir / "crlf-test.txt"
    ...
    @pytest.mark.tonio
    async def test_preserves_crlf_line_endings_after_edit(self, tmp_dir):
        test_file = tmp_dir / "crlf-preserve.txt"
    ...
    @pytest.mark.tonio
    async def test_preserves_lf_line_endings_for_lf_files(self, tmp_dir):
        test_file = tmp_dir / "lf-preserve.txt"
        test_file.write_bytes(b"first\nsecond\nthird\n")
        edit_tool = create_edit_tool(str(tmp_dir))
    ...
    @pytest.mark.tonio
    async def test_detects_duplicates_across_crlf_lf_variants(self, tmp_dir):
        test_file = tmp_dir / "mixed-endings.txt"
    ...
    @pytest.mark.tonio
    async def test_preserves_utf8_bom_after_edit(self, tmp_dir):
        test_file = tmp_dir / "bom-test.txt"
    ...
    @pytest.mark.tonio
    async def test_preserves_crlf_line_endings_and_bom_in_multi_edit_mode(self, tmp_dir):
        test_file = tmp_dir / "bom-crlf-multi.txt"
        test_file.write_bytes("\ufefffirst\r\nsecond\r\nthird\r\nfourth\r\n".encode())
        edit_tool = create_edit_tool(str(tmp_dir))

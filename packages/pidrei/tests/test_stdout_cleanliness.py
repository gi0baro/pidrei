...
def _make_dirs(tmp_path) -> tuple[str, str]:
...
class TestStdoutCleanlinessInNonInteractiveModes:
    def test_prints_version_to_stdout_when_stdout_is_redirected(self, tmp_path):
        agent_dir, project_dir = _make_dirs(tmp_path)
    ...
    def test_prints_plain_help_to_stdout_when_stdout_is_redirected(self, tmp_path):
        agent_dir, project_dir = _make_dirs(tmp_path)
    ...
    def test_keeps_stdout_empty_for_mode_json_help(self, tmp_path):
        agent_dir, project_dir = _make_dirs(tmp_path)
    ...
    def test_keeps_stdout_empty_for_p_help(self, tmp_path):
        agent_dir, project_dir = _make_dirs(tmp_path)
    ...
    def test_untrusted_project_help_still_works(self, tmp_path):
        agent_dir, project_dir = _make_dirs(tmp_path)

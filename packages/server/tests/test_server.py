...
@pytest.mark.tonio
async def test_rejects_an_overlong_derived_private_unix_bind_path(sock_dir):
...
@pytest.mark.tonio
async def test_rejects_concurrent_start_calls_without_leaking_the_unix_listener(sock_dir):
...
def test_rejects_pending_byte_limits_smaller_than_one_maximum_frame(sock_dir):
    path = str(sock_dir / "server.sock")

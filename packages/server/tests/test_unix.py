...
@pytest.mark.tonio
async def test_rejects_a_live_listener_without_unlinking_it(tmp_dir):
...
@pytest.mark.tonio
async def test_never_unlinks_a_regular_file_at_the_configured_path(tmp_dir):
    harness = Harness(tmp_dir)
    try:
        path = tmp_dir / "server.sock"
...
@pytest.mark.tonio
async def test_creates_nested_temp_parents_restricts_permissions_and_removes_its_own_socket(tmp_dir):
    harness = Harness(tmp_dir)
    try:
...
@pytest.mark.tonio
async def test_does_not_remove_a_replacement_inode_during_shutdown(tmp_dir):
    harness = Harness(tmp_dir)
    try:
        path = harness.socket_path()
        server = harness.make_server(path)
...
@pytest.mark.tonio
async def test_removes_a_genuinely_stale_socket_before_binding(tmp_dir):
    harness = Harness(tmp_dir)
    try:
...
def test_unix_server_preset_forwards_listener_and_server_options(tmp_dir):
    server = create_unix_server(
        TestServerService(), UnixServerOptions(path=str(tmp_dir / "server.sock"), server_id="preset-1")
    )

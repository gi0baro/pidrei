...
@pytest.mark.tonio
async def test_pi_client_exchanges_fragmented_framed_messages_over_a_real_unix_socket(tmp_dir):
    socket_path = str(tmp_dir / "pi.sock")
...
@pytest.mark.tonio
async def test_bounds_pending_writes_preserves_order_and_reports_remote_end_once(tmp_dir):
    socket_path = str(tmp_dir / "pi.sock")
...
@pytest.mark.tonio
async def test_pi_client_rejects_a_truncated_final_frame_from_a_real_unix_socket(tmp_dir):
    socket_path = str(tmp_dir / "pi.sock")
    listener = await net.open_unix_listener(socket_path)
...
@pytest.mark.tonio
async def test_rejects_connection_errors(tmp_dir):
    missing_path = str(tmp_dir / "missing.sock")
